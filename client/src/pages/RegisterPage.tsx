...
const RegisterPage: React.FC<Props> = ({ onLoginClick }) => {
    ...
    const isGmailValid = email.toLowerCase().trim().endsWith('@gmail.com') || email === '';
    const isPasswordLengthValid = password.length >= 8 || password === '';
    const doPasswordsMatch = password === confirmPassword || confirmPassword === '';

    // Registration is always enabled in the new system
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (!email.toLowerCase().trim().endsWith('@gmail.com')) {
            setError('Only Gmail accounts (@gmail.com) are allowed for spam prevention');
            setLoading(false);
            return;
        }
        ...
        try {
            await api.register(name, email, password, phone, parseInt(age));
            // Show success screen
            setSuccess(true);
        } catch (err: any) {
    ...
    if (success) {
        return (
            <div className="min-h-screen bg-black text-white flex items-center justify-center p-4 relative overflow-hidden">
                <div className="w-full max-w-md bg-zinc-900/50 backdrop-blur-xl border border-white/10 rounded-2xl p-8 shadow-2xl z-10 text-center animate-in zoom-in duration-300">
                    <div className="inline-flex items-center justify-center w-16 h-16 bg-green-500/20 rounded-full mb-6">
                        <CheckCircle2 className="w-8 h-8 text-green-500" />
                    </div>
                    <h2 className="text-2xl font-bold mb-4">Registration Successful!</h2>
                    <p className="text-gray-300 mb-6">
                        We have sent a verification email to <strong className="text-white">{email}</strong>.
                        <br /><br />
                        Please verify your email address to log in. Check your spam folder if you don't see it.
                    </p>
                    <button onClick={onLoginClick} className="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-3 rounded-lg transition-colors">
                        Proceed to Login
                    </button>
                </div>
            </div>
